...
- **2026-05-15 testarch-automate expansion**: 34 additional tests added to fill integration-path / edge-case coverage gaps the original 53-test suite did not address. New test classes: `TestMultiRoundCombatSequence` (3 — progressive deaths across simulated rounds, dm_turn realignment after deaths, three consecutive defeated NPCs followed by PC), `TestAllNpcsDefeatedMidRound` (4 — only-dead-NPCs returns END, defensive infinite-loop guard at 50 dead NPCs, index-past-end behavior), `TestConcurrentTurnMutations` (4 — multiple chained dm_update_npc tool calls in a single DM turn against same and different NPCs, including condition chaining), `TestPersistenceFullCombatRoundTrip` (3 — round_number/initiative_order/initiative_rolls/original_turn_queue/npc_profiles all round-trip; mutate-then-re-serialize cycle; router defeated-skip survives round-trip), `TestSheetNotificationOrdering` (3 — multi-tool-call ordering preserved, [SHEET] entries precede [DM] narrative line, error sibling does not break ordering), `TestBuildDmContextSnapshotStability` (5 — repeat invocation byte-identity, initiative-order honored over dict insertion order, orphan profiles excluded, exact AC #9 format snapshot, exploration mode produces zero combat-keyword leakage), `TestExecuteNpcUpdateEdgeCases` (10 — JSON-string args dispatch path, garbled JSON, empty/whitespace npc_name, non-int hp_change coercion, no-op confirmation cleanliness, conditions-only updates, conditions add+remove in same call, already-defeated no-op, removing absent condition), `TestDmTurnIndexAlignment` (2 — no over-advance when index already at live entry or at dm bookend). Total Story 15.7 tests: **87** (53 original + 34 expansion). All passing. Broader test suite has same 20 pre-existing failures as baseline (verified via git stash round-trip) — no new regressions. **One real bug surfaced (out of scope for 15.7 to fix)**: `deserialize_game_state()` (persistence.py:349-356) does not restore `current_initiative_index` — it always resets to 0 on reload, which would cause a saved-mid-combat campaign to replay from the start of the round on resume. Test `test_full_combat_state_round_trips_with_mutations` documents this with an explanatory note pointing at the persistence-layer cause.
