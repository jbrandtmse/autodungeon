...
Get a game running in under 5 minutes:
...
Edit `.env` and add at least one API key. The fastest path is a free [Google AI Studio](https://aistudio.google.com/) key:

```
GOOGLE_API_KEY=your_key_here
```

That's it — all agents default to `gemini-3-flash-preview` out of the box.

**3. Run**

```bash
bash dev.sh        # Linux / macOS / Git Bash
# or
.\dev.ps1          # PowerShell on Windows
```

Open **http://localhost:5173**, click **+ New Adventure**, pick a module (or go freeform), and hit **Start Autopilot** to watch the AI party play. Press **1-4** to drop into a character at any time, **Esc** to release.

### Key Features

- **Watch Mode** - AI party plays autonomously; watch the story unfold in real time
- **Drop-In Play** - Press 1-4 to take control of any character, Esc to release
...
- **Illustration Gallery** - Browse all generated artwork in a cross-session gallery with lightbox viewer, hover prompts, and bulk download
...
**Status: v2.1 - AI Scene Image Generation & Gallery complete**

All 17 epics (97 functional requirements) have been implemented across 4 major releases.
...
- [Product Requirements Document](_bmad-output/planning-artifacts/prd.md) - 97 functional requirements
...
- **API Keys**: `.env` file (or configure via the in-app Settings modal)
...
All agent models, token limits, API keys, and image generation settings can be configured from the Settings modal (press **S** in-game). Changes are saved to `user-settings.yaml` and take effect on the next turn.

### Running Servers Manually

If you prefer to start each server individually instead of using `dev.sh`:
...
The original Streamlit UI is preserved as a backward-compatible alternative:
...
This keeps the DM and background agents on Gemini Flash for speed and quality, while running PC characters locally on Ollama to reduce API costs.

**Without Ollama** (cloud-only):

All agents default to `gemini-3-flash-preview`. Just set `GOOGLE_API_KEY` in `.env` and you're ready to go. You can also mix in Claude models for PC characters if you have an Anthropic key (`ANTHROPIC_API_KEY`).
...
All generated images are collected in a cross-session gallery accessible from both the game view (press **G**) and the Adventures list (click the gallery icon on any session card).

- **Thumbnail grid** with square previews, sorted by turn number
- **Lightbox viewer** with full-size images, left/right navigation, and metadata
- **Session switcher** to browse artwork from any session
- **Download** individual images or bulk-download all as a zip archive
- **Hover tooltips** show the generation prompt and timestamp

### Keyboard Shortcuts

| Key | Action |
|-----|--------|
| **1-4** | Drop in to control character 1-4 |
| **Esc** | Release character / close lightbox / close gallery |
| **Space** | Pause or resume autopilot |
| **G** | Toggle illustration gallery |
| **S** | Open settings |
| **N** | Send nudge (suggestion to DM) |
| **Left/Right** | Navigate images in lightbox |
| **D** | Download current lightbox image |
