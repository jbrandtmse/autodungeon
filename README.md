...
All agent models, token limits, and API keys can be configured from the **Configuration** modal in the Streamlit UI (gear icon). Changes are saved to `user-settings.yaml` and take effect on the next turn.
