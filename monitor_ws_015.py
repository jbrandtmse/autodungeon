"""
WebSocket-based overnight monitor for Session 015.
Connects via Vite proxy, starts autopilot, monitors progress,
auto-restarts on failure, and generates illustrations every 30 minutes.

Run: python monitor_ws_015.py
"""
...
WS_URI = "ws://localhost:5173/ws/game/015"
API_BASE = "http://localhost:8000/api"
LOG_FILE = Path("C:/autodungeon/campaign_015_ws_monitor.log")
NARRATIVE_FILE = Path("C:/autodungeon/campaign_015_narrative.log")
TARGET_TURNS = 1000
ILLUSTRATION_INTERVAL_SECONDS = 1800  # 30 minutes

# State tracking
last_turn = 0
last_illustration_time = 0.0
...
def log_narrative(turn: int, entries: list[str]) -> None:
    """Write narrative entries to the narrative log file."""
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open(NARRATIVE_FILE, "a", encoding="utf-8") as f:
        f.write(f"\n{'=' * 60}\n")
        f.write(f"[{ts}] Round ending at turn {turn}\n")
        f.write(f"{'=' * 60}\n\n")
...
def generate_illustration(turn_number: int) -> bool:
    ...
    global total_illustrations
    turn_idx = max(0, turn_number - 3)
    url = f"{API_BASE}/sessions/015/images/generate-turn/{turn_idx}"
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as resp:
            ...
            log(f"  Illustration #{total_illustrations} requested for turn {turn_number} (task: {task_id})")
            return True
...
async def run_monitor():
    ...
    global last_turn, last_illustration_time, autopilot_restarts, total_errors, start_time

    start_time = time.time()
    last_illustration_time = start_time
    last_hourly_report = start_time

    log("=" * 60)
    log("SESSION 015 WS MONITOR STARTED")
    log(f"Target: {TARGET_TURNS} turns")
    log(f"Illustration interval: every {ILLUSTRATION_INTERVAL_SECONDS // 60} minutes")
    log("Party: Krag (Barbarian), Aurelia (Paladin), Fenwick (Ranger), Zephyra (Sorcerer)")
    log("Module: Tomb of Annihilation")
    log("PC Model: qwen3.5:35b (Ollama @ 192.168.0.123)")
    log("DM Model: gemini-3-flash-preview")
    log("=" * 60)
                ...
                while True:
                    try:
                        ...
                        if msg_type == "turn_update":
                            ...
                            # Log each agent's entry in the status log
                            n_entries = len(new_entries)
                            log(f"TURN {turn} ({n_entries} entries, {rate:.0f} turns/hr)")
                            for entry in new_entries:
                                # Extract agent name from "[AgentName]: ..." format
                                if isinstance(entry, str) and entry.startswith("["):
                                    bracket_end = entry.find("]:")
                                    if bracket_end > 0:
                                        agent_name = entry[1:bracket_end]
                                        # Log first 120 chars of content as preview
                                        preview = entry[bracket_end + 3:bracket_end + 123].replace("\n", " ")
                                        log(f"  [{agent_name}] {preview}...")

                            # Write full narrative to separate log
                            if new_entries and isinstance(turn, int):
                                log_narrative(turn, new_entries)
                            ...
                            # Time-based illustrations (every 30 min)
                            now = time.time()
                            if isinstance(turn, int) and now - last_illustration_time >= ILLUSTRATION_INTERVAL_SECONDS:
                                log(f"30-minute illustration trigger at turn {turn}...")
                                if generate_illustration(turn):
                                    last_illustration_time = now
                        ...
                        if consecutive_silence >= 10:  # 20 min of silence
                    ...
                    if time.time() - last_hourly_report >= 3600:
                        ...
                        log(f"  Illustrations generated: {total_illustrations}")
                        log("=" * 50)
